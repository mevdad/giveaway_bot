...
async def show_active_selected_give(
    ...
    for give in give_data:
        ...
        message_text = f'<b>Тип розіграшу:</b> <code>{"За коментарями" if give["type"] == "comments" else "За кнопкою"}</code>\n<b>Назва розіграшу:</b> <code>{give["name"]}</code>\n\n<b>Текст:</b>\n{give["text"]}\n\n<b>Фото:</b> <code>{"Ні" if give["photo_id"] == "False" else "Так"}</code>\n<b>Дата закінчення:</b> <code>{give["over_date"]}</code>\n<b>Капча:</b> <code>{"Так" if give["captcha"] else "Ні"}</code>\n<b>Кількість переможців:</b> <code' \
